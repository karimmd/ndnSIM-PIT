...
//============== Our Custom Defined Field ==============================
 ...
 /* int bet = this->getBetw(); // assigns random Nonce if needed
  totalLength += encoder.prependByteArray(reinterpret_cast<uint8_t*>(&bet), sizeof(bet));
  totalLength += encoder.prependVarNumber(sizeof(bet));
  totalLength += encoder.prependVarNumber(tlv::Betw);
  */
 //totalLength += prependNonNegativeIntegerBlock(encoder,tlv::Betw,getBetw());
  /*std::string pre=this->getPprefix();  
  totalLength += encoder.prependVarNumber(sizeof(pre));
  totalLength += encoder.prependVarNumber(tlv::Pprefix);
  */
  //totalLength += prependStringBlock(encoder,tlv::Pprefix,getPprefix()); 
...
totalLength += prependNonNegativeIntegerBlock(encoder,
                                                  tlv::Ccode,
                                                  getCcode());
  totalLength += prependStringBlock(encoder,tlv::CFlow,getCFlow());
...
//============Our Custom Defined Field==============
  ...
  auto val = m_wire.find(tlv::Betw);
  if (val != m_wire.elements_end()) {
    betw = readNonNegativeInteger(*val);
    ++ele;
  }  
   auto val2 = m_wire.find(tlv::Pprefix);
  if (val2 != m_wire.elements_end()) {
    pprefix = readString(*val2);
    ++ele;
  }

auto val3 = m_wire.find(tlv::Ccode);
  if (val3 != m_wire.elements_end()) {
    ccode = readNonNegativeInteger(*val3);
    ++ele;
  }

  auto val4 = m_wire.find(tlv::CFlow);
  if (val4 != m_wire.elements_end()) {
    cflow = readString(*val4);
    ++ele;
  }  

auto val5 = m_wire.find(tlv::CsVal);
  if (val5 != m_wire.elements_end()) {
    csV = readNonNegativeInteger(*val5);
    ++ele;
  }

auto val6 = m_wire.find(tlv::Pnode);
  if (val6 != m_wire.elements_end()) {
    pn = readNonNegativeInteger(*val6);
    ++ele;
  }
  ...
  for (const Block& ele : m_wire.elements()) {
    switch (ele.type()) {
      ...
      case tlv::Parameters: {
       ...
       case tlv::Betw: {
        if (lastEle >= 9) {
          BOOST_THROW_EXCEPTION(Error("Betw element is out of order"));
        }
        // TLV-VALUE is ignored
        lastEle = 9;
        break;
       ...
       case tlv::Pprefix: {
        if (lastEle >= 10) {
          BOOST_THROW_EXCEPTION(Error("Pprefix element is out of order"));
        }
        // TLV-VALUE is ignored
        lastEle = 10;
        break;
      ...
      case tlv::Ccode: {
        if (lastEle >= 11) {
          BOOST_THROW_EXCEPTION(Error("Controll Code element is out of order"));
        }
        // TLV-VALUE is ignored
        lastEle = 11;
        break;
     ...
     case tlv::CFlow: {
        if (lastEle >= 12) {
          BOOST_THROW_EXCEPTION(Error("Controll Flow element is out of order"));
        }
        // TLV-VALUE is ignored
        lastEle = 12;
        break;
        ...
        case tlv::CsVal: {
        if (lastEle >= 13) {
          BOOST_THROW_EXCEPTION(Error("Content Value element is out of order"));
        }
        // TLV-VALUE is ignored
        lastEle = 13;
        break;
        ...
        case tlv::Pnode: {
        if (lastEle >= 14) {
          BOOST_THROW_EXCEPTION(Error("Producer Node element is out of order"));
        }
        // TLV-VALUE is ignored
        lastEle = 14;
        break;
...
//============== Our Custom Defined Value ==========

Interest&
Interest::setBetw(const int& b)
{
  m_wire.reset();
  betw = b;
  return *this;
...
Interest&
Interest::setPprefix(const std::string& pre)
{
  m_wire.reset();
  pprefix = pre;
  return *this;
...
Interest&
Interest::setCcode(const int& cod)
{
  m_wire.reset();
  ccode = cod;
  return *this;
...
Interest&
Interest::setCFlow(const std::string& flow)
{
  m_wire.reset();
  cflow= flow;
  return *this;
...
Interest&
Interest::setCsVal(const int& cs)
{
  m_wire.reset();
  csV = cs;
  return *this;
...
Interest&
Interest::setPnode(const u_int32_t& pnode)
{
  m_wire.reset();
  pn = pnode;
  return *this;
...
operator<<(std::ostream& os, const Interest& interest)
{
  ...
  /*if (interest.getBetw() >= 0) {
    os << delim << "ndn.Betw=" << interest.getBetw();
    delim = '&';
  }

  if (!interest.getPprefix().empty()) {
    os << delim << "ndn.Pprefix=" << interest.getPprefix();;
    delim = '&';
  }
  */
  return os;
