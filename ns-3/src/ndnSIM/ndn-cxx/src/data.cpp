...
//=======================Our Custom Entry =============================
totalLength += prependStringBlock(encoder,tlv::ContMiss,getContentMiss());
/*
...
*/
auto val = m_wire.find(tlv::ContMiss);
  if (val != m_wire.elements_end()) {
    missContent= readString(*val);
...
Data&
Data::setContentMiss(const std::string& missCont)
{
 m_wire.reset();
  missContent = missCont;
  return *this;
}
/*
Data&
Data::setBetw(const int& b)
{
  m_wire.reset();
  betw = b;
  return *this;
...
Data&
Data::setPprefix(const std::string& pre)
{
  m_wire.reset();
  pprefix = pre;
  return *this;
